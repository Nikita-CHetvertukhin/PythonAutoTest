...
@allure.title('Создание простого Dotx')
def test_create_dotx(error_handler, logger, admin_driver, setup_create_delete_file):
    """Тест проверяет создание простого Dotx"""
    file_name, my_files_page, xpath = setup_create_delete_file
    my_files_editor_page = MyFilesEditorPage(admin_driver, logger)

    logger.info("Начало проверки создания простого dotx")
