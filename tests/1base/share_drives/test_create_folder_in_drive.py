...
@allure.title('Создание папки на общем диске')
def test_create_folder_in_drive(error_handler, logger, admin_driver, setup_create_delete_drive):
    """Тест проверяет создание папки на общем диске"""
    drive_name, my_files_page, xpath = setup_create_delete_drive
    my_files_editor_page = MyFilesEditorPage(admin_driver, logger)
    file_name = f"{get_uuid()}_folder_test_create_folder_in_drive{get_timestamp()}"

    logger.info("Начало проверки создания папки в общем диске")
    ...
    my_files_page.create_file(file_name, "Новую папку")
