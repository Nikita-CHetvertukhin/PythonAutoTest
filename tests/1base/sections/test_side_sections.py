...
from settings.variables import WORKSPACE
...
@pytest.mark.skipif(
    ...
    reason="Сборка с CLM, раздел можно проверить через запуск с аргументом '-m workflow' или '-m workflow_smoke'"
)
@pytest.mark.parametrize("tabs, columns", [
    (["Доступные мне", "Недавние", "Корзина"], ["check", "Название", "Дата изменения", "Автор", "Комментарий"]),
    (["Мои файлы"], ["check", "Название", "Дата изменения", "Автор", "Комментарий"]),
...
def test_side_sections(request, error_handler, logger, admin_driver, tabs, columns):
