...
def test_check_full_acces_docx(error_handler, logger, admin_driver, expert_driver, setup_create_delete_file):
    ...
    my_files_editor_expert.check_acces_in_editor(acces_level="Полный доступ", setting_type="Настройки таблицы", text="justTable")
    my_files_editor_expert.check_acces_in_editor(acces_level="Полный доступ", setting_type="Настройки изображения", element_class="drawing")
    # Проверка возможности создания первой переменной на вкладке "схема"
    my_files_editor_expert.open_side_panel_in_doc("Схема")
    assert my_files_editor_expert.create_first_variable("test"), "Не удалось добавить переменную, несмотря на то, что уровень доступа 'Полный доступ'"
    ...
    my_files_editor_expert.check_acces_in_header_section(acces_level="Полный доступ", section_name="Вставка")
    my_files_editor_expert.check_acces_in_header_section(acces_level="Полный доступ", section_name="Макет")
    if coloboration_box:
        my_files_editor_expert.check_acces_in_header_section(acces_level="Полный доступ", section_name="Рецензирование")