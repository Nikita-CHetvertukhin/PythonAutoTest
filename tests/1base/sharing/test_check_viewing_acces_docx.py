...
def test_check_viewing_acces_docx(error_handler, logger, admin_driver, expert_driver, setup_create_delete_file):
    ...
    logger.info("Начало проверки уровня доступа 'Просмотр'")
    ...
    result = my_files_expert.get_action_availability("docx", "Просмотр",coloboration_box)
    ...
    my_files_editor_expert.check_acces_in_editor(acces_level="Просмотр", setting_type="Настройки абзаца", text="justParagraph")
    my_files_editor_expert.check_acces_in_editor(acces_level="Просмотр", setting_type="Настройки таблицы", text="justTable")
    my_files_editor_expert.check_acces_in_editor(acces_level="Просмотр", setting_type="Настройки изображения", element_class="drawing")
    # Проверка невозможности создания первой переменной на вкладке "схема"
    my_files_editor_expert.open_side_panel_in_doc("Схема")
    assert not my_files_editor_expert.create_first_variable("test"), "Первая переменная успешно добавлена в docx, несмотря на то, что уровень доступа - Просмотр"
    # Проверка действий на вкладках тулбара
    my_files_editor_expert.check_acces_in_header_section(acces_level="Просмотр", section_name="Конструктор")
    my_files_editor_expert.check_acces_in_header_section(acces_level="Просмотр", section_name="Главная")
    my_files_editor_expert.check_acces_in_header_section(acces_level="Просмотр", section_name="Вставка")
    my_files_editor_expert.check_acces_in_header_section(acces_level="Просмотр", section_name="Макет")
    if coloboration_box:
        my_files_editor_expert.check_acces_in_header_section(acces_level="Просмотр", section_name="Рецензирование")