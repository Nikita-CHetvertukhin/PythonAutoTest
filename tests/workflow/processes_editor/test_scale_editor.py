...
@pytest.fixture
def setup_test_scale_editor(request, logger, admin_driver):
...
def test_scale_editor(error_handler, logger, admin_driver, setup_test_scale_editor):
    """Тест проверяет создание, перемещение и соединение фигур в редактоер Workflow."""
    process_name, workflows_page, workflow_editor_page = setup_test_scale_editor
