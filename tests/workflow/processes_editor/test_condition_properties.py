...
@pytest.fixture
def setup_test_condition_properties(request, logger, admin_driver):
...
def test_condition_properties(error_handler, logger, admin_driver, setup_test_condition_properties):
    """Тест проверяет доступность для изменения и сохранение после перезагрузки страницы свойств фигуры 'Условие'"""
    process_name, workflows_page, workflow_editor_page = setup_test_condition_properties

    logger.info("Начало проверки свойств фигуры 'Условие'")
    ...
    first_shape = workflow_editor_page.add_shape("Условие") # Создание первой фигуры
