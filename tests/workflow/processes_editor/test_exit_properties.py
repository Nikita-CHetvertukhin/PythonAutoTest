...
@pytest.fixture
def setup_test_exit_properties(request, logger, admin_driver):
...
def test_exit_properties(error_handler, logger, admin_driver, setup_test_exit_properties):
    """Тест проверяет доступность для изменения и сохранение после перезагрузки страницы свойств фигуры 'Выход'"""
    process_name, workflows_page, workflow_editor_page = setup_test_exit_properties
