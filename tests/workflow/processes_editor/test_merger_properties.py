...
@pytest.fixture
def setup_test_merger_properties(request, logger, admin_driver):
...
def test_merger_properties(error_handler, logger, admin_driver, setup_test_merger_properties):
    """Тест проверяет доступность для изменения и сохранение после перезагрузки страницы свойств фигуры 'Слияние'"""
    process_name, workflows_page, workflow_editor_page = setup_test_merger_properties

    logger.info("Начало проверки свойств фигуры 'Слияние'")
    ...
    first_shape = workflow_editor_page.add_shape("Слияние") # Создание первой фигуры
