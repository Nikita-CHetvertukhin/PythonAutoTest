...
def test_process_properties(error_handler, logger, admin_driver, setup_test_process_properties):
    ...
    workflow_editor_page.role_properties(role_name="test_role",action="create",checkboxes="OFF", access_level="Комментатор",users={USER1_LOGIN})
    ...
    assert workflow_editor_page.role_properties(role_name="test_role",action="check",checkboxes=None, access_level="Комментатор [◦◦◦◦◦◦◦]",users={USER1_LOGIN})
