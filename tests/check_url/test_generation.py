...
def test_generateDatabaseSchema(error_handler, logger, admin_driver):
    ...
    if not generation_page.check_error(False, GenerationLocators.GENERATION_ERROR, False):
        error_handler.handle_exception(MinorIssue("Генерация выполнена, но присутсвуют ошибки"))
