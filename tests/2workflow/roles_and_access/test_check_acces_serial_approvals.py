...
# # Массивный тест-кейс
# # Все наборы доступов для проверок
# # Стартовый при создании задачи
...
# logins_and_access_default_check = [(USER5_LOGIN, "Полный доступ"),(USER2_LOGIN, "Нет доступа"),(USER3_LOGIN, "Нет доступа"),(USER4_LOGIN, "Нет доступа")]
# # После смены исполнителя
# logins_and_access1 = [(USER5_LOGIN, "Просмотр"),(USER2_LOGIN, "Нет доступа"),(USER3_LOGIN, "Нет доступа"),(USER4_LOGIN, "Нет доступа"),(USER1_LOGIN, "Полный доступ")]
# # После согласования первой УЗ
# logins_and_access2 = [(USER5_LOGIN, "Просмотр"),(USER2_LOGIN, "Рецензирование"),(USER3_LOGIN, "Нет доступа"),(USER4_LOGIN, "Нет доступа"),(USER1_LOGIN, "Просмотр")]
# # После отправки на доработку второй УЗ
# logins_and_access3 = [(USER5_LOGIN, "Просмотр"),(USER2_LOGIN, "Просмотр"),(USER3_LOGIN, "Нет доступа"),(USER4_LOGIN, "Нет доступа"),(USER1_LOGIN, "Просмотр")]
# # После доработки
# logins_and_access4 = [(USER5_LOGIN, "Просмотр"), (USER2_LOGIN, "Просмотр"), (USER3_LOGIN, "Нет доступа"), (USER4_LOGIN, "Нет доступа"), (USER1_LOGIN, "Полный доступ")]
...
@allure.title('Проверка доступов при последовательном согласовании')
def test_check_acces_serial_approvals(
    ...
    """Тест проверяет доступы при последовательном согласовании"""
    ...
    logger.info(f"Начало проверки доступов для системного процесса: 'Последовательное согласование'")
    ...
    assert (result := my_files_page.share_access(action="check", logins_and_access=logins_and_access_default_check)) is True, f"Ошибка: Доступы {result[1]} не совпадают с ожидаемыми {logins_and_access_default_check}."
    ...
    # user2_my_tasks_page.add_required_comment(text_comment=f"sending_to_rework_fron_{USER2_LOGIN}")
    ...
    # my_tasks_page.find_click_header_menu("Мои задачи")
    # my_tasks_page.find_click_side_menu("Мои задачи")
    # my_tasks_page.complete_task(task_name, subtask_massive=[(3, "Доработка", "Согласовать")])
    # my_tasks_page.find_click_header_menu("Документы")
    # my_tasks_page.find_click_side_menu("Мои файлы")
    # my_files_page.right_click_and_select_action(file_name, "Открыть")
    # time.sleep(1)  # TODO Заменить на динамическое ожидание
    # xpath.find_clickable(MyFilesEditorLocators.ACCESS_BUTTON, timeout=10).click()
    # assert (result := my_files_page.share_access(action="check", logins_and_access=logins_and_access4)) is True, f"Ошибка: Доступы {result[1]} не совпадают с ожидаемыми {logins_and_access4}."
