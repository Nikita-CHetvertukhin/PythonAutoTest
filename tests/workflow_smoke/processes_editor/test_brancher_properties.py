...
def test_brancher_properties(error_handler, logger, admin_driver, setup_create_delete_process):
    ...
    # P.s. наверное можно оптимизировать универсальным методом add и check для всех свойств, но пока не стал из-за того что в некоторых методах отличается более одного свойства