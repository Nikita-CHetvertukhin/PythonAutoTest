...
LOGGING_CONFIG = {
    ...
    # Сторонние библиотеки логируют на DEBUG каждую HTTP-команду драйверу и полный ответ
    # (тело + заголовки) — при открытом root это заливает файл/отчёт их внутренней кухней.
    # Поднимаем именно им порог, не трогая наши собственные логгеры.
    "loggers": {
    ...
    # Фильтрация по уровню происходит на хендлере (file=DEBUG),
    # поэтому здесь держим порог открытым — иначе logger.debug(...) нигде не сработает.
    "root": {
        "level": "DEBUG",
        "handlers": ["file"]  # Использует обработчик для записи логов в файл
    }
}
